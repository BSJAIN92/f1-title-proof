...
Set `DASHBOARD_PASSWORD` to a private password of at least 12 characters and `DASHBOARD_SESSION_SECRET` to a random value of at least 32 characters. The dashboard is available at `/dashboard`; its signed login lasts one day. `RATE_LIMIT_SECRET` hashes request addresses for abuse controls, and `CRON_SECRET` protects the daily 90-day retention job. All three secrets must be different random values of at least 32 characters. Country reporting starts when the dashboard release is deployed because earlier visit events did not store country codes.
