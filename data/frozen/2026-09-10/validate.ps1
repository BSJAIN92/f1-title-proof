...
Assert-Equal $results.events.Count 19 'completed sessions plus reconciliation entry'
Assert-Equal (($results.events | Where-Object session -eq 'race').Count) 13 'completed races'
Assert-Equal (($results.events | Where-Object { $_.session -eq 'sprint' -and $_.event -ne 'official-standings-reconciliation-2026-09-10' }).Count) 5 'completed Sprints'
Assert-Equal (($results.events.rows | Measure-Object).Count) 401 'completed classification and reconciliation rows'
Assert-Equal $countback.qualifying_events.Count 13 'qualifying fallback events'
Assert-Equal (($countback.qualifying_events.rows | Measure-Object).Count) 286 'qualifying rows'
Assert-Equal (($manifest.futureLineup.drivers | Measure-Object).Count) 22 'future drivers'
Assert-Equal $manifest.remainingSessions.Count 11 'remaining points-paying sessions'
