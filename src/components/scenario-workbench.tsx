...
export function ScenarioWorkbench({data}:{data:ProductData}){
 ...
 function clearComparison(){setTargetId("");setRivalId("");setResult(null);setError(null)}
 function returnToStandings(){if(targetId&&rivalId)void fetch("/api/comparison-return",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({kind,targetId,rivalId,dataVersion:data.dataVersion,ruleVersion:data.ruleVersion}),keepalive:true}).catch(()=>undefined);clearComparison()}
 return <div className={`workbench ${targetId?"show-detail":"show-master"}`}><StandingsPanel season={season} kind={kind} remaining={data.remaining} rows={data.standings[kind]} busy={busy} onKind={next=>{setKind(next);clearComparison()}} onCompare={(target,rival)=>void compare(target,rival)}/>{targetId?<ScenarioDossier kind={kind} targetId={targetId} rivalId={rivalId} result={result} busy={busy} error={error} onBack={returnToStandings} onRetry={()=>void compare()}/>:null}</div>
}
