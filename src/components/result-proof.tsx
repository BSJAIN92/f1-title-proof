...
function summaryRows(rows:readonly PositionRow[]):SummaryRow[]{
 const grouped=new Map<string,SummaryRow>();
 for(const row of rows){const finish=row.target.replace(/ \(\d+\)$/,""),key=`${row.session}:${finish}`,current=grouped.get(key);if(current)current.count+=1;else grouped.set(key,{session:row.session,finish,count:1})}
